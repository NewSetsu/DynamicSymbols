...
struct DoubleObject : public VarBase
{
public:
    DoubleObject();
...
public:
    ...
    virtual void Reset() override { m_num = 0.0; }
    ...
    virtual const int GetIntVar() override { return static_cast<int>(m_num); }
...
public:
    // 该函数最后应该只能由内存池调用
    void SetVal(const double val) { m_num = val; }
